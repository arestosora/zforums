<template>
  <div class="flex h-screen w-full relative">
    <div :class="{ 'blur-sm': isLoading }" class="flex-1 flex items-center justify-center bg-gray-200 overflow-hidden">
      <img src="../assets/image.png" alt="Imagen de la izquierda" class="w-full h-full object-cover" />
    </div>
    <div :class="{ 'blur-sm': isLoading }" class="flex-1 flex items-center justify-center">
      <form @submit.prevent="register" class="max-w-lg w-full p-12">
        ...
        <div class="mb-6">
          ...
          <img v-if="avatarFile" :src="avatarFile" alt="Profile Picture Preview"
            class="mt-4 w-32 h-32 object-cover rounded border border-gray-300" />
        </div>
  ...
  setup() {
    ...
    const register = async () => {
      ...
      try {
        ...
        }, 3000); // Keep the loading screen for 3 seconds
      }
    };
